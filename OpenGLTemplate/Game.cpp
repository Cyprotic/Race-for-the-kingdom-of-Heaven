...
void Game::Initialise() 
{
	...
	*m_pViewMatrix = glm::lookAt(vEye, vView, vUp);
	...
	m_pAngelMesh = new COpenAssetImportMesh;
	m_pWingsMesh = new COpenAssetImportMesh;
	...
	m_pAngelMesh->Load("resources\\Angel\\angel.obj");  // Downloaded from http://www.psionicgames.com/?page_id=24 on 24 Jan 2013
	m_pWingsMesh->Load("resources\\wings\\3d-model.obj");  // Downloaded from http://www.psionicgames.com/?page_id=24 on 24 Jan 2013
...
void Game::Render() 
{
	...
	pMainProgram->SetUniform("material1.Ma", glm::vec3(0.0f, 0.0f, 1.f));	// Ambient material reflectance
	pMainProgram->SetUniform("material1.Md", glm::vec3(0.0f, 0.0f, 1.f));	// Diffuse material reflectance
	pMainProgram->SetUniform("material1.Ms", glm::vec3(0.0f, 0.0f, 1.f));	// Specular material reflectance
	...
	// Set light lamp 2
	glm::vec4 lightPosition4 = glm::vec4(offsetLeft[5].x, 15, offsetLeft[5].z, 1); // Position of light source *in world coordinates*
	pMainProgram->SetUniform("light4.position", viewMatrix * lightPosition4); // Position of light source *in eye coordinates*
	pMainProgram->SetUniform("light4.La", glm::vec3(0.0f));		// Ambient colour of light
	pMainProgram->SetUniform("light4.Ld", glm::vec3(1.0f));		// Diffuse colour of light
	pMainProgram->SetUniform("light4.Ls", glm::vec3(1.0f));		// Specular colour of light
	pMainProgram->SetUniform("material4.Ma", glm::vec3(0.0f, 0.0f, 1.f));	// Ambient material reflectance
	pMainProgram->SetUniform("material4.Md", glm::vec3(0.0f, 0.0f, 1.f));	// Diffuse material reflectance
	pMainProgram->SetUniform("material4.Ms", glm::vec3(0.0f, 0.0f, 1.f));	// Specular material reflectance
	pMainProgram->SetUniform("material4.shininess", 15.0f);		// Shininess material property
	pMainProgram->SetUniform("light4.direction", glm::normalize(viewNormalMatrix * glm::vec3(0, -1, 0)));		// Direction material property
	pMainProgram->SetUniform("light4.exponent", 20.0f);		// Exponent material property
	pMainProgram->SetUniform("light4.cutoff", 30.0f);       // Cutoff material property

	// Set light car + ambient
	glm::vec4 lightPosition3 = glm::vec4(p.x, p.y, p.z, 1); // Position of light source *in world coordinates*
	pMainProgram->SetUniform("light3.position", viewMatrix * lightPosition3); // Position of light source *in eye coordinates*
	if (nightEnable == 1)
		pMainProgram->SetUniform("light3.La", glm::vec3(0.2f));		// Ambient colour of light
	else
		pMainProgram->SetUniform("light3.La", glm::vec3(0.8f));		// Ambient colour of light
	...
	pMainProgram->SetUniform("material3.Ma", glm::vec3(1.0f,1.0f, 1.f));	// Ambient material reflectance
	pMainProgram->SetUniform("material3.Md", glm::vec3(1.0f, 1.0f, 1.f));	// Diffuse material reflectance
	pMainProgram->SetUniform("material3.Ms", glm::vec3(1.0f, 1.0f, 1.f));	// Specular material reflectance
	pMainProgram->SetUniform("material3.shininess", 15.0f);		// Shininess material property
	pMainProgram->SetUniform("light3.direction", glm::normalize(viewNormalMatrix * glm::vec3(T.x, -0.2f, T.z)));		// Direction material property
	pMainProgram->SetUniform("light3.exponent", 20.0f);		// Exponent material property
	pMainProgram->SetUniform("light3.cutoff", 30.0f);       // Cutoff material property
	...
	//	modelViewMatrixStack.Push();
	//	modelViewMatrixStack.Translate(glm::vec3(x, 130.0f, z));
	//	//modelViewMatrixStack.Rotate(glm::vec3(0.0f, 1.0f, 0.0f), 180.0f);
	...
	modelViewMatrixStack.Push();
	modelViewMatrixStack.Translate(glm::vec3(500.0f, 115.0f, -100.0f));
	//modelViewMatrixStack.Rotate(glm::vec3(0.0f, 1.0f, 0.0f), 180.0f);
	modelViewMatrixStack.Scale(glm::vec3(15.f, 5.f, 10.f));
	pMainProgram->SetUniform("matrices.modelViewMatrix", modelViewMatrixStack.Top());
	pMainProgram->SetUniform("matrices.normalMatrix", m_pCamera->ComputeNormalMatrix(modelViewMatrixStack.Top()));
	...
	modelViewMatrixStack.Push();
	modelViewMatrixStack.Translate(glm::vec3(-450.0f, 115.0f, -200.0f));
	//modelViewMatrixStack.Rotate(glm::vec3(0.0f, 1.0f, 0.0f), 180.0f);
	modelViewMatrixStack.Scale(glm::vec3(15.f, 5.f, 10.f));
	pMainProgram->SetUniform("matrices.modelViewMatrix", modelViewMatrixStack.Top());
	pMainProgram->SetUniform("matrices.normalMatrix", m_pCamera->ComputeNormalMatrix(modelViewMatrixStack.Top()));
	...
	modelViewMatrixStack.Push();
	modelViewMatrixStack.Translate(glm::vec3(-200.0f, 115.0f, -300.0f));
	//modelViewMatrixStack.Rotate(glm::vec3(0.0f, 1.0f, 0.0f), 180.0f);
	modelViewMatrixStack.Scale(glm::vec3(15.f, 5.f, 10.f));
	pMainProgram->SetUniform("matrices.modelViewMatrix", modelViewMatrixStack.Top());
	pMainProgram->SetUniform("matrices.normalMatrix", m_pCamera->ComputeNormalMatrix(modelViewMatrixStack.Top()));
	...
	modelViewMatrixStack.Push();
	modelViewMatrixStack.Translate(glm::vec3(50.0f, 115.0f, -200.0f));
	//modelViewMatrixStack.Rotate(glm::vec3(0.0f, 1.0f, 0.0f), 180.0f);
	modelViewMatrixStack.Scale(glm::vec3(15.f, 5.f, 10.f));
	pMainProgram->SetUniform("matrices.modelViewMatrix", modelViewMatrixStack.Top());
	pMainProgram->SetUniform("matrices.normalMatrix", m_pCamera->ComputeNormalMatrix(modelViewMatrixStack.Top()));
	...
	modelViewMatrixStack.Push();
	modelViewMatrixStack.Translate(glm::vec3(600.0f, 115.0f, -100.0f));
	//modelViewMatrixStack.Rotate(glm::vec3(0.0f, 1.0f, 0.0f), 180.0f);
	modelViewMatrixStack.Scale(glm::vec3(15.f, 5.f, 10.f));
	pMainProgram->SetUniform("matrices.modelViewMatrix", modelViewMatrixStack.Top());
	pMainProgram->SetUniform("matrices.normalMatrix", m_pCamera->ComputeNormalMatrix(modelViewMatrixStack.Top()));
	...
	// Render the Angel
	modelViewMatrixStack.Push();
	modelViewMatrixStack.Translate(glm::vec3(250, 0.1f, -214));
	modelViewMatrixStack.Scale(glm::vec3(0.5f));
	pMainProgram->SetUniform("matrices.modelViewMatrix", modelViewMatrixStack.Top());
	pMainProgram->SetUniform("matrices.normalMatrix", m_pCamera->ComputeNormalMatrix(modelViewMatrixStack.Top()));
	m_pAngelMesh->Render();
	modelViewMatrixStack.Pop();
	...
	// Render the Wings
	modelViewMatrixStack.Push();
	modelViewMatrixStack.Translate(glm::vec3(790, 190.f, -80));
	modelViewMatrixStack.Rotate(glm::vec3(1.0f, 0.0f, 0.0f), glm::radians(-90.f));
	modelViewMatrixStack.Rotate(glm::vec3(0.0f, 0.0f, 1.0f), glm::radians(-90.f));
	...
	for (int i = 0; i < offsetLeft.size(); i = i + 5)
	{
		...
		modelViewMatrixStack.Push();
		modelViewMatrixStack.Translate(glm::vec3(offsetLeft[i].x + 1, offsetLeft[i].y, offsetLeft[i].z));
	...
	for (int i = 0; i < offsetRight.size(); i = i + 2)
	{
		...
		modelViewMatrixStack.Push();
		modelViewMatrixStack.Translate(glm::vec3(offsetRight[i].x - 1, offsetRight[i].y, offsetRight[i].z));
		modelViewMatrixStack.Rotate(glm::vec3(0, 1, 0), glm::radians(90.f));
		modelViewMatrixStack.Rotate(glm::vec3(0, 1, 0), glm::atan(T.x, T.z));
		modelViewMatrixStack.Scale(2.f);
		pMainProgram->SetUniform("matrices.modelViewMatrix", modelViewMatrixStack.Top());
	...
	// Render the sphere
	modelViewMatrixStack.Push();
		modelViewMatrixStack.Translate(glm::vec3(-10.0f, 20.0f, -150.0f));
...
void Game::DisplayFrameRate()
{
	...
	if (m_framesPerSecond > 0) {
		...
		m_pFtFont->LoadFont("resources\\fonts\\Ubuntu-Medium.ttf", 40);
		// FPS
		...
		// Positions
		m_pFtFont->ReleaseFont();
		m_pFtFont->LoadFont("resources\\fonts\\Farro-Medium.ttf", 40);
		m_pFtFont->Render(20, height - 500, 20, "DEVELOPMENT VERSION", m_pCamera->GetPosition().x);
		...
		m_pFtFont->ReleaseFont();
		m_pFtFont->LoadFont("resources\\fonts\\Roboto-Medium.ttf", 40);
		// Camera controls
		...
		m_pFtFont->Render(620, height - 80, 20, "Left Camera: 4");
		m_pFtFont->Render(620, height - 100, 20, "Right Camera: 5");
		m_pFtFont->Render(620, height - 120, 20, "Top Camera: 6");
		m_pFtFont->Render(620, height - 140, 20, "Free Camera: 7");
		// Car controls
		m_pFtFont->Render(620, height - 160, 20, "Foward: W");
		m_pFtFont->Render(620, height - 180, 20, "Slowdown: S");
		m_pFtFont->Render(620, height - 200, 20, "Left: A");
		m_pFtFont->Render(620, height - 220, 20, "Right: D");
		// Effects
		m_pFtFont->Render(620, height - 240, 20, "Fog: F");
		m_pFtFont->Render(620, height - 260, 20, "Blink: B");
		m_pFtFont->Render(620, height - 280, 20, "Night: N");
		m_pFtFont->ReleaseFont();
