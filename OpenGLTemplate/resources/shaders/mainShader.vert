...
vec3 BlinnPhongSpotlightModel(vec4 p, vec3 n, LightInfo light, MaterialInfo material)
{
	...
	if (angle < cutoff) 
	{
		...
		if (sDotN > 0.0)
		specular = light.Ls * material.Ms * pow(max(dot(h, n), 0.0),
		material.shininess);
...
// Save the world position for rendering the skybox
	...
	if (fogEnable == 1)
		{
			float dist = abs(vEyePosition.z);
			float fogFactor = (100 - dist) / (100 - 50);
			fogFactor = clamp(fogFactor, 0.0, 1.0);
			//vec3 shadeColor = PhongModel(vEyePosition, normalize(vEyeNorm));
			// Apply the Phong model to compute the vertex colour
			vColour = BlinnPhongSpotlightModel(vEyePosition, normalize(vEyeNorm), light1, material1) +
			BlinnPhongSpotlightModel(vEyePosition, normalize(vEyeNorm), light3, material3) +
			BlinnPhongSpotlightModel(vEyePosition, normalize(vEyeNorm), light4, material4);
			vColour = mix (vec3(0.0f, 0.0f, 0.0f), vColour, fogFactor);
		}
	else
		{
			vColour = BlinnPhongSpotlightModel(vEyePosition, normalize(vEyeNorm), light1, material1) +
	...
	{
		// Blinking
		vColour = vColour * (1 + 0.5*sin(500*t)); 
	}
